...
cartApp.controller("addToCartCtrl", function ($scope, $http){
	 ...
	 $scope.retrieveCart = function (username) {
	    ...
	    $scope.refreshCartItems = function () {
	        $http.get('http://localhost:8080/ClothFrontend/refreshcart/'+$scope.username).success(function (data) {
	           $scope.cart=data;
	        });
	    };
	    
	    $scope.addtocart = function(itemId){
	    	 $http.put('http://localhost:8080/ClothFrontend/addtocart/'+itemId).success(function () {
	             alert("Item added to the cart!")
	         });
	    }
	    
	    $scope.removeItemFromCart=function(cartid){
	    	$http.delete('http://localhost:8080/ClothFrontend/removeItemFromCart/' +cartid).success(function(){
